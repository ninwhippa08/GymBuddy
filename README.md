...
**Two movement pools do not meet the variety target, and the shortfall is now
five entries.** `docs/coverage-matrix.md` is generated by the test suite and
...
unwanted, no longer carry the target. What is left is `primary :: hinge/pull-h
:: power` (short 3) and `primary :: jump :: (any)` (short 2), and both are
closeable by authoring or by tagging, which is why neither was exempted.
`docs/design-running-programming.md` §11.0.
