...
**Two movement pools do not meet the variety target, and the shortfall is 11
entries.** `docs/coverage-matrix.md` is generated by the test suite and
prints the raw shortfall. Nine of the 11 are in a pool that did not exist as a
pool until 2026-09-09: opening the sprint day's start slot (below) made seven
accessory-tier starts prescribable for the first time, and they arrived
carrying the variety target that the two sprint pools beside them had been
exempted from three days earlier. **Nothing has decided whether they keep it**,
and until something does the matrix will keep printing `short 9` — which is the
correct behaviour for an open question, and the same standing position as the
heavy-hinge pool below. It stood at 94 until 2026-09-06, almost all of it in
