...
**No movement pool is short of its variety target any more, and the raw
shortfall reads zero for the first time.** `docs/coverage-matrix.md` is
generated by the test suite and prints it. It stood at 94 until 2026-09-06,
almost all of it in
...
**The sixth pull was mined against the measurement's advice, and the
measurement was right about the entries and wrong about one joint.** NASM's
technique playlist is 77 titles of gym-side work: no sprints, no starts, no
running, and so nothing for the only short pool new movements can reach. The
recommendation was to author nothing. Overruled, it produced **eleven entries**
— two wrist movements, a bodyweight squat the library had never had, a squat
thrust, a jumping jack, a bench dip, a high cable curl, two planks, a supine
rotation drill and a neck stretch. All eleven are drawn in a 4,900-session
sweep, the worst session stays at 68 minutes, and **no shortfall moved**, which
is what the pre-flight predicted. One thing it got wrong is worth more than the
rest: the neck stretch was nearly declined as outside the joint vocabulary, and
`neck` has been in that vocabulary since the soreness map was built, held by
just two entries. Checking a vocabulary before ruling something outside it
costs one grep. `docs/design-library-expansion.md` §30.

**The tool was wrong in the direction that reaches the data, and it is the
third time the matcher has been the defect.** This channel brands every title
— *"How to do a Plank | Proper Form & Technique | NASM"* — and the score
measures precision against all the words, so nine words of channel sank every
match under the threshold. Run as it was, the diff recognised **1** title and
proposed **72** candidates; with the branding stripped it recognises 46 and
proposes 28. **Forty-five titles the library already held were reported as
new.** That is worse than the 57 missed duplicates of the fourth pull, because
the two errors are not symmetric: a missed duplicate gets dropped by eye, while
a false candidate gets authored and the library then holds one movement under
two names. The fix defines boilerplate by what it is — a pipe-delimited segment
repeated across at least 30% of a playlist — rather than by naming channels, and
`tests/playlist-tool.test.mjs` pins it, including the case that keeps the four
earlier pulls' results byte-identical.

**The starts pool was settled on 2026-09-11, and settling it found the counter
at fault rather than the library.** Opening the sprint day's start slot made
seven starts prescribable and handed them a variety target of 16, which they
had been printing a shortfall against ever since. Two measurements decided it.
The pool is the **least repeated in the library** — 0.60 repeats per entry per
twelve-week block, against 1.55 for the run pool that an earlier section called
the least repetitive this project had worked on. And the target it misses is
counting the wrong thing: the athlete's rule is no repeat inside **16
sessions**, the sprint day arrives every 8.5 sessions, so a pool drawn only on
that day needs **two** entries to clear that horizon and it holds seven. The
pool is now exempt, and the raw shortfall falls from 11 to 2.

The exemption is recorded on a **different ground** from the seven beside it,
because it is a different argument. Those are pools where the movements do not
exist. Roughly fifteen solo start positions do exist, so this target could have
been approached; it is exempt because the target is mis-scaled and the
repetition is already the smallest in the library. **The mis-scaling is general
and is deliberately not fixed**: no day type here arrives more often than every
5.3 sessions, so every main-work pool is targeted at four to eight times its own
horizon. Correcting that would make a derived document depend on a stochastic
...
**The last shortfall was closed on 2026-09-11, and the matrix now reads zero.**
The heavy hinge pool had stood at 14 of 16 since early September and was the one
pool an earlier pass called genuinely closeable by authoring. It is not, for two
independent reasons. What remains in the catalogue is bar position, which this
library treats as a dose rather than a movement. The two entries with a real
training case behind them, the jump shrug and the mid-thigh pull, cannot be
priced at all: the coefficient field asserts a ratio between two maxes, and the
literature prescribes these as a percentage of a *different* lift's max. Nobody
tests a jump shrug to failure.

**The second reason only became knowable once the repeat interval was
measured.** This pool is power day slot B, and at 14 entries it returns a
movement on the same median as the max-strength slot that draws 45 — three turns
of that day type, 24 sessions. The two missing entries would change nothing
perceptible. A schema change to unlock them was offered and declined, correctly:
it would buy exactly two movements in a family where three already exist.

Zero does not mean the library is finished. It means no pool is short of a
target the project still believes in, that nine exemptions each carry their
argument at the point of exemption, and that **the coverage matrix has stopped
